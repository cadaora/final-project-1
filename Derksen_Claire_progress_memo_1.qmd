...
```{r}
#| label: missingness
miss_var_summary(NSDUH_2020_college) #add slice?
...
```
The variables that were removed were reserved for people under 18. So our data set of college students will have little to no percentage missing. For the remaining variables I will have to look through the code book in order to determine whether the data is worth keeping. 

## Potential data issues
I'll need to merge the two datasets. The most tedious part will be turning the integer variables into factor variables using the code book. (Explain general code 999). If there are still a lot of variables after I remove the non-compatable ones using the compatibility chart I will start with removing the variables with the highest portion of missingness. Another potential issue I noticed was processing time. It takes a long time for the >100MB .txt files to load. I solved this by creating two new .rds files for each year in order to shorten future loading times.



## Misc

I will start with importing the excel compatibility chart for 2019-2020 and converting it into a tibble. I could then use the tibble to select what variables to keep. After that I'll assess missingness again and
